...
var Dashboard = function() {
    ...
    var productPercentage = function() {
        if ($('#m_chart_product_percentage').length == 0) {
            return;
        }

        Morris.Donut({
            element: 'm_chart_product_percentage',
            data: [{
                    label: "Capacitors",
                    value: 10
                },
    ...
    //The percentages of the products in your store
    ...
    //The percentages of the products in your store
    var categoryPercentage = function() {
        if ($('#m_chart_category_change').length == 0) {
            return;
        }

        $.post('api/index.php', {action:'getCategories', shop:shop}, function(data){

            try{
                ret = JSON.parse(data);

                Morris.Donut({
                    element: 'm_chart_category_change',
                    data: [{
                            label: "Electronics",
                            value: 37
                        },
    ...
    var datatableProducts = function() {
        ...
        var datatable = $('#products_datatable').mDatatable({
            data: {
                type: 'remote',
                source: {
                    read: {
                        url: 'api/index.php?action=getProducts&shop='+shop,
                    }
                },
                pageSize: 10,
                saveState: {
